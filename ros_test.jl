...
function path_callback(msg::TrajectoryDiscretized, pub_obj::Publisher{Marker})
    ...
    @debugtask begin
        ...
        # if isnothing(obstacles[]) # in master branch we have no obstacles as polytopes
        #     println("no obstacles")
        if isnothing(traj_received[])
            println("no spline")
        else 
...
function main()
    ...
    sub1 = Subscriber{TrajectoryDiscretized}("/spline_traj_samples", path_callback, (pub,), queue_size=10)
